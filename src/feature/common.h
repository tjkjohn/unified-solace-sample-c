...
#define REPLAY_START_STRING      "\t-R, --replay=replay Replay Start Location String (BEGINNING or RFC3339 time stamtp).\n"
...
/**
 * @struct contextThreadInfo
 * Holds context information.
 */
typedef struct contextThreadInfo
{

    solClient_opaqueContext_pt context_p;   /**< the context */

    volatile int    contextThreadStarted;   /**< 1 when the thread has been started */

    volatile int    stopContextThread;      /**< set to 1 to stop the context thread */

    int             rc;                     /**< return code from solClient_context_processEvents */
